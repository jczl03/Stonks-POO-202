...
public class ConClient implements ActionListener{
	...
	public ConClient(Client client, ConsultClient query, PanelClient frame) {
		...
		fillCombos();
                llenarTabla();
                this.panel.addChangeListener(e->actualizarTabla(e));
	...
	public void llenarTabla(){
            this.panel.cargarMatriz(query.getTableAsMatriz());
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource() == panel.getBtnAnadir()){
			client.setNombre(panel.getTxtNombre().getText().trim().toUpperCase());
			client.setDireccion(panel.getTxtDireccion().getText());
			client.setAt(panel.getTxtAt().getText());
                        
                        if(query.getId(client.getNombre())<0){
				...
				clean();
                            ...
                            JOptionPane.showMessageDialog(null, "No se ha podido realizar la operación, ya existe un cliente con el nombre "+client.getNombre());
                            clean();
